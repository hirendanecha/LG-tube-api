...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in LatinaGirls tube channel by the LatinaGirls tube Admin.
  To access your channel, log into your LatinaGirls tube account,click on the
  LatinaGirls tube icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `LatinaGirls tube has approved your Connection application.`;
    } else {
      msg = `LatinaGirls tube has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
